...
const ModuleManager = {
    ...
    cleanupModule(moduleName) {
        if (this.moduleStates[moduleName]) {
            ...
            // No elimines el estado del módulo, solo marca como no inicializado
    ...
    loadModule(moduleName) {
        ...
        // Si el módulo ya fue inicializado antes, solo necesitamos volver a cargar el HTML
        if (this.moduleStates[moduleName].initialized) {
            this.reloadModuleHTML(moduleName, mainContainer);
        } else {
    ...
    reloadModuleHTML(moduleName, mainContainer) {
        fetch(`modules/${moduleName}/${moduleName}.html`)
            .then(response => response.text())
            .then(html => {
                mainContainer.innerHTML = html;
                // Volver a ejecutar la función de inicialización del módulo
                const initFunctionName = `inicializarModulo${moduleName.split('-').map(word => 
                    word.charAt(0).toUpperCase() + word.slice(1)).join('')}`;
                if (typeof window[initFunctionName] === 'function') {
                    window[initFunctionName]();
            ...
            .catch(error => console.error(`Error al recargar el HTML del módulo ${moduleName}:`, error));
    },
    
    loadModuleFiles(moduleName, mainContainer) {
        // ... (el resto del código de carga de archivos permanece igual)
    }
};
