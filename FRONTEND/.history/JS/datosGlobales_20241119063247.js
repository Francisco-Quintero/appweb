...
const datosGlobales = {
    ...
    agregarVariante: function(variante) {
        variante.id = this.generarId(this.variantes);
        this.variantes.push(variante);
        this.guardarEnLocalStorage();
    },

    actualizarVariante: function(varianteActualizada) {
        const index = this.variantes.findIndex(v => v.id === varianteActualizada.id);
        if (index !== -1) {
            this.variantes[index] = varianteActualizada;
            this.guardarEnLocalStorage();
        }
    },

    eliminarVariante: function(id) {
    ...
    guardarEnLocalStorage: function() {
        ...
        localStorage.setItem('variantes', JSON.stringify(this.variantes));
    ...
    cargarDesdeLocalStorage: function() {
        ...
        const variantesGuardadas = localStorage.getItem('variantes');
        ...
        if (variantesGuardadas) this.variantes = JSON.parse(variantesGuardadas);
