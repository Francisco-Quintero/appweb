    ...
    console.log('Iniciando carga del módulo de proveedores');
        ...
        function esperarDatosGlobales() {
            return new Promise((resolve) => {
                if (window.datosGlobales && typeof window.datosGlobales.guardarEnLocalStorage === 'function') {
                    console.log('datosGlobales ya está disponible');
                    resolve();
                } else {
                    console.log('Esperando a que datosGlobales esté disponible...');
                    const intervalo = setInterval(() => {
                        if (window.datosGlobales && typeof window.datosGlobales.guardarEnLocalStorage === 'function') {
                            console.log('datosGlobales ahora está disponible');
                    ...
                    setTimeout(() => {
                        clearInterval(intervalo);
                        console.error('Tiempo de espera agotado para datosGlobales');
...
async function initProveedores() {
    ...
    try {
        ...
        if (!window.datosGlobales) {
            console.error('Error: datosGlobales no está disponible después de esperar');
            return;
        }
    ...
    // Modificamos la inicialización para manejar async
