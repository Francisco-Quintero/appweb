...
(function() {
    ...
    function guardarInventario(e){
        ...
        if (idInventario) {
            const index = inventario.findIndex(i => i.idInventario === parseInt(idInventario));
            if (index !== -1) {
                inventario[index] = inventarioNuevo;
                console.log('Inventario actualizado:', inventario);
            }
        } else {
            inventario.push(inventarioNuevo);
            console.log('Nuevo inventario agregado:', inventario);
        }

        guardarEnLocalStorage();
        cargarInventario();
        document.getElementById('formularioStock').style.display = 'none';
    }
    ...
    function actualizarStock(id) {
        ...
        //document.getElementById('nombreProducto').value = item.idProducto;
