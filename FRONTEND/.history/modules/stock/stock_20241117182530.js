    ...
    function cargarInventario() {
        ...
        cuerpoTabla.innerHTML = inventario.map(item => `
            <tr class="${item.stock <= item.puntoReorden ? 'alerta-stock' : ''}">
                ...
                <td>
                    <button onclick="actualizarStock(${item.idInventario})" class="btn btn-secundario">
                        Actualizar
                    </button>
                </td>
            </tr>
    ...
    function verificarAlertasReorden() {
        inventario.forEach(item => {
            if (item.stock <= item.puntoReorden) {
                ...
                // Aquí podrías implementar notificaciones visuales o enviar alertas
            }
        });
    }
