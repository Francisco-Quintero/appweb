...
(function() {
    ...
    function guardarInventario(){
        ...
        const nombre = document.getElementById('idInventario').value;

        const invent = inventario.find(p => p.idInventario === nombre);
        if (!invent) {
        ...
        const nuevoInventario = {
            idInventario: invent ? parseInt(invent) : Date.now(),
        ...
        if (producto) {
            const index = productos.findIndex(p => p.idProducto === parseInt(producto));
            if (index !== -1) {
    ...
    function configurarEventListeners() {
        ...
        document.getElementById('formularioStock').addEventListener('submit', manejaguardarInventariorEnvioFormulario);
        document.getElementById('formularioStock').addEventListener('submit', );
