const moduloCompras = (function() {
    ...
    function cargarCompras(comprasFiltradas = comprasData) {
    ...
    function mostrarResultadosBusqueda(resultados) {
        ...
        divResultados.innerHTML = resultados.map(resultado => {
            const productoStr = encodeURIComponent(JSON.stringify(resultado.producto));
            return `
                <div class="producto-resultado" data-producto='${productoStr}'>
                    ...
                    <div class="variantes-lista">
                        ${resultado.variantes.map(variante => {
                            const varianteStr = encodeURIComponent(JSON.stringify(variante));
                            return `
                                <div class="variante-item" 
                                     data-producto='${productoStr}' 
                                     data-variante='${varianteStr}'
                                     onclick="moduloCompras.seleccionarProducto(this.dataset.producto, this.dataset.variante)">
    ...
    function seleccionarProducto(productoStr, varianteStr) {
        try {
            const producto = JSON.parse(decodeURIComponent(productoStr));
            const variante = JSON.parse(decodeURIComponent(varianteStr));
            
            const busquedaInput = document.getElementById('busquedaProducto');
            const unidadMedidaInput = document.getElementById('unidadMedida');
            
            if (busquedaInput && unidadMedidaInput) {
                busquedaInput.value = `${producto.Nombre} - ${variante.descripcion}`;
                busquedaInput.dataset.productoId = producto.idProducto;
                busquedaInput.dataset.varianteId = variante.id_variante;
                unidadMedidaInput.value = variante.unidad_medida;
...
document.addEventListener('DOMContentLoaded', moduloCompras.inicializar);
