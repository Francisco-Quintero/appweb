...
(function() {
    ...
    function cargarCompras() { //el nuevo
        ...
        const contenidoTabla = compras.length === 0
            ...
            : compras.map(compra => {
                   return `
            <tr>
                <td>${compcompraras.id}</td>
                <td>${compras.proveedor}</td>
                <td>${compras.fecha}</td>
                <td>${compras.producto}</td>
                <td>$${compras.total.toFixed(2)}</td>
                <td>
                    <span class="estado-compra estado-${compras.estado.toLowerCase()}">
                        ${compras.estado}
                    </span>
                </td>
                <td>
                    <button onclick="window.verDetallesCompra(${compras.id})" class="btn btn-secundario">
                        <i data-lucide="eye"></i>
                        Ver más
                    </button>
                </td>
    ...
    function cargarProveedores() {
        c
        `).join('');onsole.log('Cargando proveedores');
