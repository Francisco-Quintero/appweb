(function() {
    ...
    function initCompras() {
        ...
        // Luego intentar configurar event listeners
        try {
    ...
    function cargarDatosIniciales() {
        ...
        if (typeof window.datosGlobales === 'undefined') {
            console.error('window.datosGlobales no está definido. Verifica que se haya cargado correctamente.');
            return;
        }
    ...
    function cargarCompras() {
        ...
        if (!cuerpoTabla) {
            console.error('Elemento con id "cuerpoTablaCompras" no encontrado. Verifica que exista en el HTML.');
            return;
        }
    
        try {
            if (comprasData.length === 0) {
                cuerpoTabla.innerHTML = '<tr><td colspan="6">No hay compras registradas</td></tr>';
                return;
            }
    ...
    function configurarEventListeners() {
        ...
        for (const [key, element] of Object.entries(elements)) {
            if (!element) {
                throw new Error(`Elemento ${key} no encontrado`);
            }
        }

        // Configurar event listeners solo si todos los elementos existen
        elements.busquedaProveedor.addEventListener('input', (e) => {
        ...
        elements.categoriaFiltro.addEventListener('change', buscarProductos);
        ...
        elements.btnAgregarProducto.addEventListener('click', agregarProductoACompra);
