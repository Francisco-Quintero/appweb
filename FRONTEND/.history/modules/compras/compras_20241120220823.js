...
(function() {
    ...
    function buscarProductos(termino) {
        ...
        return productos.filter(producto =>{
            ...
            return (
                ...
                (producto.unidadMedida && producto.unidadMedida.toLowerCase().includes(termino.toLowerCase())) ||
                (producto.valorMedida && producto.valorMedida.toLowerCase().includes(termino.toLowerCase()))
            );
        } catch (error) {
    ...
    function mostrarResultadosBusqueda(resultados) {
        ...
        resultados.forEach(producto => {
            ...
            div.textContent = `${producto.Nombre} - ${producto.Descripcion} - ${producto.valorMedida} ${producto.unidadMedida}`;
    ...
    function seleccionarProducto(producto) {
        ...
        proveedor function obtenerProveedor(){
            const selectProveedor = document.getElementById('proveedor');
            const proveedorSeleccionadoId = selectProveedor.value; // Obtiene el ID del proveedor seleccionado
            console.log('Proveedor seleccionado ID:', proveedorSeleccionadoId);
        }
    ...
    function agregarOEditarProducto() {
        ...
        const suministro = {
            ...
            totalCompra: precioCompraTotal,
            Cantidad: cantidad,
            idProveedor: compras.proveedor.id,
        }; 
        //crear una constante para mandarla a una variable para que actualize el inventario de la idProducto
        //se guarda el el precioUnnitario final, la cantidad,  

        if (indiceEditando !== null) {
            compras[indiceEditando] = suministro;
            indiceEditando = null;
            document.getElementById('btnAgregarProducto').textContent = 'Agregar Producto';
        } else {
            compras.push(suministro);
        }
...
function actualizarTablaProductos() {
    ...
    compras.forEach((compra, indice) => {
        const producto = productos.find(p => p.id === compra.idProducto);
        ...
        tr.innerHTML = `
            ...
            <td class="texto-derecha">$${compra.totalCompra.toFixed(2)}</td>
    ...
    function calcularTotales() {
        ...
        const subtotal = compras.reduce((sum, item) => sum + item.totalCompra, 0);
        const impuesto = parseFloat(document.getElementById('impuesto').value) || 0;
