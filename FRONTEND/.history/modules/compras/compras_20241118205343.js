(function() {
    ...
    // Wait for both DOM and datosGlobales to be ready
    function inicializarCompras(datosGlobales) {
        // Ensure DOM is loaded first
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => initCompras(datosGlobales));
        } else {
            initCompras(datosGlobales);
        }
    }

    function initCompras(datosGlobales) {
        console.log('Inicializando módulo de compras con datos:', datosGlobales);
        ...
        const elements = {
            ...
            selectProveedor: document.querySelector('#proveedor')
        };
        ...
        // Initialize data only if elements exist
        if (elements.cuerpoTablaCompras && elements.selectProveedor) {
            cargarDatosIniciales(datosGlobales);
        } else {
            console.error('Elementos críticos no encontrados. No se pueden cargar los datos.');
        }
    }
    ...
    // Initialize when both DOM and datosGlobales are ready
    if (window.datosGlobales) {
        inicializarCompras(window.datosGlobales);
    } else {
