...
(function() {
    ...
    function iniciarCarga() {
        if (retryCount >= MAX_RETRIES) {
            console.error('Error: No se pudo cargar el módulo después de varios intentos');
            return;
        }

        const cuerpoTabla = document.getElementById('cuerpoTablaCompras');
        if (!cuerpoTabla) {
            console.warn(`Intento ${retryCount + 1}/${MAX_RETRIES}: Elemento cuerpoTablaCompras no encontrado`);
            retryCount++;
            setTimeout(iniciarCarga, 500); // Aumentar el tiempo entre intentos
            return;
        }

        // Si llegamos aquí, el elemento existe y podemos proceder
        console.log('Elemento cuerpoTablaCompras encontrado, iniciando carga de datos');
