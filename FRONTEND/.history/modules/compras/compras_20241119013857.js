if (!window.moduloCompras) {
    const moduloCompras = (function() {
        ...
        function configurarEventListeners() {
            ...
            // Agregar event listener para los clics en las variantes
            document.getElementById('resultadosBusqueda').addEventListener('click', handleVarianteClick);
        }

        function cargarCompras(comprasFiltradas = null) {
            ...
            const comprasAMostrar = comprasFiltradas || comprasData;
            ...
            cuerpoTabla.innerHTML = comprasAMostrar.map(compra => `
                <tr>
                    <td>${compra.id}</td>
                    <td>${compra.fecha}</td>
        ...
        function buscarProductos() {
            ...
            console.log('Buscando:', termino); // Debug log
            ...
            console.log('Resultados encontrados:', resultados); // Debug log
        ...
        function mostrarResultadosBusqueda(resultados) {
            ...
            divResultados.innerHTML = resultados.map(producto => `
                <div class="producto-resultado" data-id-producto="${producto.idProducto}">
                    <div class="producto-info" style="cursor: pointer; padding: 8px;">
                        ${producto.Nombre}
                    </div>
                    ${variantes
                        ...
                        .map(variante => `
                            <div class="variante-item" 
                                 ...
                                 style="cursor: pointer; padding: 4px 8px; margin-left: 12px;">
        ...
        function handleVarianteClick(event) {
            const varianteItem = event.target.closest('.variante-item');
            if (!varianteItem) return; // Si no se hizo clic en una variante, no hacemos nada

            const idProducto = parseInt(varianteItem.dataset.idProducto);
            const idVariante = parseInt(varianteItem.dataset.idVariante);

            console.log('Click detectado en variante:', idProducto, idVariante); // Debug log
            seleccionarProducto(idProducto, idVariante);
        }

        function seleccionarProducto(idProducto, idVariante) {
            console.log('Ejecutando seleccionarProducto con:', idProducto, idVariante); // Debug log
            ...
            console.log('Producto seleccionado exitosamente'); // Debug log
        }
        ...
        function actualizarTablaProductos() {
            ...
            tbody.innerHTML = productosCompra.map((producto, index) => `
                <tr>
                    ...
                    <td>
                        <button onclick="moduloCompras.editarProducto(${index})" class="btn btn-secundario">Editar</button>
                        <button onclick="moduloCompras.eliminarProducto(${index})" class="btn btn-peligro">Eliminar</button>
                    </td>
                </tr>
            `).join('');
            calcularTotales();
