...
function actualizarTablaProductos() {
    ...
    tbody.innerHTML = '';
    const proveedor = productos.filter(proveedor => 
        proveedor.id.toLowerCase().includes(termino.toLowerCase())
    );
    compras.forEach((compra, indice) => {
        ...
        tr.innerHTML = `
            ...
            <td>${compra.unidad}</td>
            <td>${compra.nombre}</td>
            <td class="texto-derecha">$${compra.precioCompraTotal.toFixed(2)}</td>
            <td class="texto-derecha">$${compra.precioUnitarioFinal.toFixed(2)}</td>
